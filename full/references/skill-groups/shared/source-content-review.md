...
Sources and OCR caches stay immutable; reviews are checksum-bound views.

Compare server-provided variants and submit exact, unique replacements through
`rule_import(review_text)` or
`module_review(submit_transcript)`. A text-only Agent uses `cross_text` when two
...
Text-only Agents may consume the reviewed result later, but cannot claim vision.
...
For a damaged 2014 statblock field, call `render_page`, then
`recover_statblock` with exact page, slot, printed name, and corrections. A
...
Route by locked edition. `recover_statblock` accepts only 2014 grammar. Reviewed
